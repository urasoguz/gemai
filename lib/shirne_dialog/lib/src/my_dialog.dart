...
enum IconType { none, success, error, warning, info, help }
...
class ShirneDialog {
      ...
      [
        ...
        (button ?? localStyle?.primaryBuilder ?? primaryButtonBuilder).call(
          ...
          Padding(
            ...
            child: Text(buttonText ?? local.buttonConfirm, style: buttonStyle),
          ),
        ),
      ],
      title: title,
  ...
  Future<T?> alertModal<T>(
    ...
    if (mStyle?.scrollable ?? false) {
      ...
      body = Padding(padding: mStyle!.contentPadding, child: body);
    }
    if (withClose) {
      body = Stack(
        children: [
  ...
  EntryController dropdown(
    ...
    controller = overlayModal(
      ...
      wrapperBuilder: (context, child) => GestureDetector(
        ...
        child: Container(color: backdropColor, child: child),
      ),
      animate: AnimationConfig.fadeIn,
      leaveAnimate: AnimationConfig.fadeOut,
    );
