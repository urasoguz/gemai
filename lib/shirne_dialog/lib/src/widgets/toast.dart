...
class _ToastWidgetState extends State<ToastWidget> {
  ...
  void initState() {
    super.initState();
    style =
        (widget.style ?? MyDialog.theme.toastStyle ?? const ToastStyle())
            .bottomIfNoAlign();
    if (widget.inGroup) {
  ...
  Widget build(BuildContext context) {
    // Bu Text, overflow sorununu çözmek için aşağıda Flexible içinde kullanılacak
    ...
    if (widget.icon != null) {
      ...
      child = IconTheme(
        data: (widget.style?.iconTheme ?? const IconThemeData.fallback())
            .copyWith(color: widget.style?.foregroundColor ?? Colors.white),
        child: Row(
          ...
          children: [
            ...
            Flexible(
              // ❗Buradaki Flexible overflow'u önlüyor
              child: messageText,
            ),
          ],
        ),
      ...
      child: AnimatedSlide(
        ...
        child: ConstrainedBox(
          constraints: BoxConstraints(
            maxWidth:
                MediaQuery.of(context).size.width * 0.9, // %90 genişlik sınırı
          ),
          child: Container(
